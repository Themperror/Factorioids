...
VS_OUT main(float4 pos : POS, uint vertexID : SV_VertexID)
{
	...
	if (vertexID == 0)
	{
		vsOut.uv = float2((spriteIndexX + 1.0) * spriteWidth, spriteIndexY * spriteWidth);
	}
	else if (vertexID == 1 || vertexID == 3)
	{
		vsOut.uv = float2(spriteIndexX * spriteWidth, spriteIndexY * spriteWidth);
	}
	else if (vertexID == 2 || vertexID == 5)
	{
		vsOut.uv = float2((spriteIndexX + 1.0) * spriteWidth, (spriteIndexY + 1.0) * spriteWidth);
	}
	else
	{
