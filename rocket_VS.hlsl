...
VS_OUT main(float4 pos : POS, uint vertexID : SV_VertexID)
{
	...
	if (vertexID == 0)
	{
		vsOut.uv = float4(0,0, spriteIndexX * spriteWidth, spriteIndexY * spriteHeight);
	}
	else if (vertexID == 1 || vertexID == 3)
	{
		vsOut.uv = float4(1,0, (spriteIndexX + 1.0) * spriteWidth, spriteIndexY * spriteHeight);
	}
	else if (vertexID == 2 || vertexID == 5)
	{
		vsOut.uv = float4(0,2, spriteIndexX * spriteWidth, (spriteIndexY + 1.0) * spriteHeight);
	}
	else
	{
		vsOut.uv = float4(1,2, (spriteIndexX + 1.0) * spriteWidth, (spriteIndexY + 1.0) * spriteHeight);
	}
